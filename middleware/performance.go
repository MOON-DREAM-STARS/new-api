...
func checkSystemPerformance() *types.NewAPIError {
	...
	if !common.IsSystemStatusFresh(status) || !common.IsSystemStatusCurrent(status) {
		return nil
	}

	// 检查 CPU
	if config.CPUThreshold > 0 && status.CPUOverloaded {
	...
	// 检查内存
	if config.MemoryThreshold > 0 && status.MemoryOverloaded {
		return types.NewErrorWithStatusCode(
	...
	// 检查磁盘
	if config.DiskThreshold > 0 && status.DiskOverloaded {
		return types.NewErrorWithStatusCode(
